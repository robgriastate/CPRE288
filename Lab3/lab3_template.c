...
//#warning "Unimplemented function: char read_shaft_encoder(void)"	// delete this line after implementing
	...
	static unsigned char old_value = 3;		// Step 2: Based on how you decided to mask PORTB in step 1, what would new_value be if the switch is in a groove?
	...
	if (old_value == 3) {			// Step 3: Enter here what you did for step 2.
...
int main(void) {
	...
	while(1){
		...
		sum += read_shaft_encoder();
		lcd_printf( "Sum: %d Button %d", sum, button);
	}
}
